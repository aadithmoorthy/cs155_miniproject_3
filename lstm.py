...
model.add(LSTM(200, input_shape=(40,39)))
...
print ("Learning")
checkpointer = ModelCheckpoint(filepath='deep_shakespeare_adam.h5', verbose=1, save_best_only=True, monitor='loss')
hist = model.fit(encoded_input, encoded_output, nb_epoch=100, batch_size=100, callbacks=[checkpointer])
model = load_model('deep_shakespeare_adam.h5')
...
for t in temps:
    ...
    for epochs in epochs_list:
        ...
        checkpointer = ModelCheckpoint(filepath='deep_shakespeare_'+str(t)+'_'+str(epochs)+'_adam_b100.h5', verbose=1, save_best_only=True, monitor='loss')
        hist = model.fit(encoded_input, encoded_output, nb_epoch=epochs, batch_size=100, callbacks=[checkpointer])

        model = load_model('deep_shakespeare_'+str(t)+'_'+str(epochs)+'_adam_b100.h5', custom_objects={'t':t})
